                ...
                {!rx.isReferral && (
                  <>
                    ...
                    {drugs.map((d: any, i: number) => (
                      <div key={i} onClick={() => setRx({ ...rx, selectedDrug: i })} style={{ padding: 16, borderRadius: 10, cursor: 'pointer', color: text, background: rx.selectedDrug === i ? 'rgba(59,130,246,0.12)' : surfaceAlt, border: `2px solid ${rx.selectedDrug === i ? accent : border}`, marginBottom: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                          <div>
                            <span style={{ fontWeight: 700, fontSize: 14, color: text }}>{d.drugName || d.drug}</span>
                            {d.brandName && <span style={{ fontSize: 12, color: muted, marginLeft: 8 }}>({d.brandName})</span>}
                          </div>
                        ...
                        {d.dosageForm && <div style={{ fontSize: 12, color: dim, marginBottom: 4 }}>{d.dosageForm}</div>}
                        <div style={{ fontSize: 13, color: muted, marginBottom: 4 }}>{d.sig}</div>
                        <div style={{ fontSize: 12, color: dim }}>{d.clinicalNotes || d.notes}</div>
                        <div style={{ display: 'flex', gap: 16, marginTop: 8, fontSize: 12, color: muted }}><span>Qty: {d.qty}</span><span>Days: {d.supplyDays || d.supply}</span>{(d.odbEligible || d.odb_eligible) && <span style={{ color: success }}>ODB ✓</span>}</div>
                      </div>
                    ))}
